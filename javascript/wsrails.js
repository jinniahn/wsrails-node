...
var Channel, Event, HttpConnection, WebSocket, WebSocketConnection, WebSocketRails,
...
WebSocketRails = (function() {
  function WebSocketRails(url, use_websockets) {
    ...
    if (!(this.supports_websockets() && this.use_websockets)) {
      this._conn = new HttpConnection(url, this);
    } else {
      this._conn = new WebSocketConnection(url, this);
    }
  ...
  WebSocketRails.prototype.trigger = function(event_name, data, success_callback, failure_callback) {
    var event;
    event = new Event([event_name, data, this.connection_id], success_callback, failure_callback);
  ...
  WebSocketRails.prototype.subscribe_private = function(channel_name) {
    var channel;
    if (this.channels[channel_name] == null) {
      channel = new Channel(channel_name, this, true);
      this.channels[channel_name] = channel;
      return channel;
    } else {
      return this.channels[channel_name];
  ...
  WebSocketRails.prototype.pong = function() {
    var pong;
    pong = new Event(['websocket_rails.pong', {}, this.connection_id]);
...
exports.WebSocketRails = WebSocketRails;
