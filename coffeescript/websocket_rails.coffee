...
class WebSocketRails
  constructor: (@url, @use_websockets = true) ->
    ...
    unless @supports_websockets() and @use_websockets
      @_conn = new HttpConnection url, @
       ...
       @_conn = new WebSocketConnection url, @
  ...
  trigger: (event_name, data, success_callback, failure_callback) =>
    event = new Event( [event_name, data, @connection_id], success_callback, failure_callback )
  ...
  subscribe_private: (channel_name) =>
    unless @channels[channel_name]?
      channel = new Channel channel_name, @, true
      @channels[channel_name] = channel
      channel
    else
      @channels[channel_name]
  ...
  pong: =>
    pong = new Event( ['websocket_rails.pong',{},@connection_id] )
    @_conn.trigger pong
