...
class Channel
  ...
  constructor: (@name,@_dispatcher,@is_private) ->
    ...
    event = new Event( [event_name, {data: {channel: @name}},@_dispatcher.connection_id], @_success_launcher, @_failure_launcher)
    @_dispatcher.trigger_event event
    @_callbacks = {}

  destroy: () =>
    event_name = 'websocket_rails.unsubscribe'
    event =  new Event( [event_name, {data: {channel: @name}}, @_dispatcher.connection_id] )
    @_dispatcher.trigger_event event
    @_callbacks = {}
  ...
  trigger: (event_name, message) =>
    event = new Event( [event_name, {channel: @name, data: message}, @_dispatcher.connection_id] )
    @_dispatcher.trigger_event event
